...
- **未読のみ（only_unread）**：未読メールのみを対象にする

複数の条件を組み合わせる場合、**全ての条件を満たす**メールが処理対象になります（AND結合）。

### 実行アクション

検索条件に一致したメールに対して、以下の処理を順次実行できます：

1. **転送（forward_to）**：指定したメールアドレスに転送します。
   - **曜日制限（forward_days）**：特定の曜日にのみ転送を実行するように設定できます。
2. **既読化（mark_as_read）**：メールを既読にします。
3. **ゴミ箱へ移動（is_trash）**：メールをゴミ箱へ移動します。
